  ...
  public passes (): boolean {
    ...
    for (const key in this.rules) {
      ...
      this.rules[key].split('|').forEach((rule: string) => {
        const [ruleKey, extraInformation] = rule.split(':');

        const ruleObject: Rule = RuleFactory.getInstance().createRule(ruleKey, this.rulePaths);
        const tempValid = ruleObject.validate(this.data[key], extraInformation);
